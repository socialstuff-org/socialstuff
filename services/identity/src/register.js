...
async function register(req, res) {
  ...
  const addLoginTokenSql = 'INSERT INTO tokens (id_user, token, expires_at) VALUES (unhex(?), unhex(?), DATE_ADD(NOW(), INTERVAL 1 DAY));';
  try {
    await db.query(addUserSql, [id, req.body.username, passwordHash, req.body.public_key]);
    await db.query(addLoginTokenSql, [id, token.replace(/-/g, '')]);
    await db.commit();
